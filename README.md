...
- `quantization_ppl_experiment.py`: 主实验脚本，支持LLaMA和OPT模型的weight-only量化和PPL评测
- `utils.py`: 工具函数，包含模型和分词器加载功能
- `quant_funcs.py`: 量化核心函数，包含pseudo_quantize_tensor等基础量化操作
- `quant_wrapper_simple.py`: 简化的量化包装器，专门用于weight-only量化

## 使用方法

### 基础用法
```bash
python quantization_ppl_experiment.py --model_path /path/to/model --datasets wikitext
```

### 自定义量化配置
```bash
# 测试4位和8位量化，使用per-channel量化
python quantization_ppl_experiment.py \
